...
public class CornerManager {
     ...
     */
    @PublicAPI
    public void drawText(GuiGraphicsExtractor graphics, String text, Corner corner) {
     ...
     */
    @PublicAPI
    public void drawText(GuiGraphicsExtractor graphics, String text, Corner corner, int xOffset, int yOffset, boolean dropShadow) {
        ...
        if (corner == Corner.TOP_RIGHT) {
            ...
            boolean areEffectsHidden = Minecraft.getInstance().screen.showsActiveEffects();
        ...
        if (corner == Corner.TOP_CENTER)
            x = (int) DrawText.centerX(0, GuiUtil.getGuiWidth(), text);
