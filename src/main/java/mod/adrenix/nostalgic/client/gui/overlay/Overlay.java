...
public class Overlay extends Screen implements RelativeLayout, WidgetHolder, ParentHolder, TooltipManager {
    /* Builders */
    ...
    public int getInsideX() {
        return this.getX() + (this.hasBorder ? 8 : 0);
    }
    ...
    public int getInsideY() {
        return this.getY() + (this.hasBorder ? 15 : 0);
    }
    ...
    public int getInsideEndX() {
        return this.getX() + this.width - (this.hasBorder ? 8 : 0);
    }
    ...
    public int getInsideEndY() {
        return this.getY() + this.height - (this.hasBorder ? 8 : 0);
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        boolean isCustomPressed = this.builder.onKeyPressInstructions.stream()
                .anyMatch(instructions -> instructions.test(this, event));
        ...
        if (KeyboardUtil.isEsc(event.key())) {
            this.close();
            return true;
        }
    ...
    public boolean charTyped(CharacterEvent event) {
        if (this.getFocused() != null)
            return this.getFocused().charTyped(event);
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.parentScreen != null) {
            ...
            this.parentScreen.extractRenderState(graphics, -1, -1, partialTick);
    ...
    /**
     * Handler method for extracting this overlay's background.
     *
     * @param graphics    A {@link GuiGraphicsExtractor} instance.
     * @param mouseX      The x-coordinate of the mouse cursor.
     * @param mouseY      The y-coordinate of the mouse cursor.
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (this.builder.hasShadow)
            graphics.fill(0, 0, winWidth, winHeight, this.builder.shadowColor.get());
        ...
        if (this.borderless() && !this.builder.outlineColor.isEmpty()) {
            ...
            graphics.outline(x, y, w, h, this.builder.outlineColor.get());
        }
    }
    ...
    private void renderBorder(GuiGraphicsExtractor graphics) {
        ...
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, ModSprite.OVERLAY, this.x, this.y, this.width, this.height);
    }
    ...
    private void center() {
        ...
        this.x = (int) MathUtil.center(this.width, GuiUtil.getGuiWidth());
        this.y = (int) MathUtil.center(this.height, GuiUtil.getGuiHeight());
    ...
    private void offScreenCheck() {
        ...
        if (this.x > GuiUtil.getGuiWidth() - 5)
            this.x = GuiUtil.getGuiWidth() - 26;

        if (this.y < 0)
            this.y = 0;

        if (this.y > GuiUtil.getGuiHeight())
            this.y = GuiUtil.getGuiHeight() - 16;

        if (this.width > GuiUtil.getGuiWidth() - 10)
            this.width = GuiUtil.getGuiWidth() - 10;

        if (this.height > GuiUtil.getGuiHeight() - 10)
