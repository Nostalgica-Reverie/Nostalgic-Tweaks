...
public class Embed extends DynamicWidget<EmbedBuilder, Embed>
     ...
     */
    @PublicAPI
    public boolean isWidgetClicked(MouseButtonEvent event) {
        if (this.isInvisible() || event.button() != 0)
            return false;

        boolean isMouseOverScrollbar = this.widgets.scrollbars.stream()
                .filter(DynamicWidget::isVisible)
                .anyMatch(scrollbar -> scrollbar.isMouseOver(event.x(), event.y()));

        if (this.isScrollbarHeld() || isMouseOverScrollbar)
            return false;

        return this.widgets.embedded.stream().anyMatch(widget -> widget.isMouseOver(event.x(), event.y()));
    }
     ...
     * clicked, then use {@link #isWidgetClicked(MouseButtonEvent)}.
     *
     * @param event@return Whether the mouse click context is valid for this embed.
     */
    @PublicAPI
    public boolean isValidClick(MouseButtonEvent event) {
        if (this.isInvisible())
            return false;

        return event.button() == 0 && MathUtil.isWithinBox(event.x(), event.y(), this.x, this.y, this.width, this.height);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (this.builder.mouseClicked != null && this.builder.mouseClicked.accept(event, doubleClick))
            return true;

        if (this.widgets.scrollbars.stream().anyMatch(scrollbar -> scrollbar.mouseClicked(event, doubleClick)))
            return true;

        boolean isWidgetClicked = this.isEventListened(widget -> {
            if (this.widgets.embedded.contains(widget) && !this.isMouseInsideWindow(event.x(), event.y()))
                return false;
        ...
        if (this.isMouseInsideWindow(event.x(), event.y())) {
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        ...
        if (this.builder.mouseReleased != null && this.builder.mouseReleased.accept(event))
            return true;

        if (this.isEventListened(widget -> widget.mouseReleased(event)))
            return true;
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        ...
        if (this.builder.mouseDragged != null && this.builder.mouseDragged.accept(event, dragX, dragY))
            return true;

        if (this.isEventListened(widget -> widget.mouseDragged(event, dragX, dragY)))
            return true;
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        if (this.builder.keyPressed != null && this.builder.keyPressed.accept(event))
            return true;

        if (this.isEventListened(widget -> widget.keyPressed(event)))
            return true;
    ...
    public boolean keyReleased(KeyEvent event) {
        ...
        if (this.builder.keyReleased != null && this.builder.keyReleased.accept(event))
            return true;

        if (this.isEventListened(widget -> widget.keyReleased(event)))
            return true;
    ...
    public boolean charTyped(CharacterEvent event) {
        ...
        if (this.builder.charTyped != null && this.builder.charTyped.accept(event))
            return true;

        if (this.isEventListened(widget -> widget.charTyped(event)))
            return true;
    ...
    protected void renderWidgets(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.enableScissor(
                this.scissor.startX().applyAsInt(this), this.scissor.startY().applyAsInt(this),
                this.scissor.endX().applyAsInt(this), this.scissor.endY().applyAsInt(this)
        );
        ...
        if (this.areScrollbarsVisible()) {
            ...
            graphics.fill(sX, sY, eX, eY, Color.SONIC_SILVER.get());
        }
