...
public class NostalgicPauseScreen extends PauseScreen implements DynamicScreen<NostalgicPauseScreen> {
    ...
    public NostalgicPauseScreen() {
        ...
        this.layout = PauseLayout.LAN;//CandyTweak.OLD_PAUSE_MENU.get();
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        if (!this.isLayoutSet) {
            if (true/*CandyTweak.REMOVE_EXTRA_PAUSE_BUTTONS.get()*/) { //TODO
                this.clearWidgets();
                this.init();
            }
        ...
        graphics.centeredText(this.font, this.title, this.width / 2, this.height / 4 - 20, 0xFFFFFFFF);
    }
}
